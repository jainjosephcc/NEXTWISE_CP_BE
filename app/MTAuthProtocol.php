...
class MTAuthProtocol  
  ...
  private function SendAuthStart($login, $is_crypt, &$auth_answer)
    {
      $apiversion = env('WebAPIVersion');
    //--- send first request, with login, webapi version
    $data = array(MTProtocolConsts::WEB_PARAM_VERSION => $apiversion, MTProtocolConsts::WEB_PARAM_AGENT => $this->m_agent, MTProtocolConsts::WEB_PARAM_LOGIN => $login, MTProtocolConsts::WEB_PARAM_TYPE => 'MANAGER', MTProtocolConsts::WEB_PARAM_CRYPT_METHOD => $is_crypt
