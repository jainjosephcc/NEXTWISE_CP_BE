...
use App\MTUserProtocol;

define("WebAPIVersion", 2190);
//--- web api date
define("WebAPIDate", "21 Oct 2019");
class MTWebAPI
  {
  /**
  ...
  public function Connect($ip, $port, $timeout, $login, $password)
	...
	$_SESSION['connection'] = $this->m_connect;
...
//Echo '<pre>';
 //print_r($this->m_connect);exit;
   // $groupu = new MTGroupProtocol($this->m_connect);
   //  $totalw = $groupu->GroupGet($group, $groupu);
   //echo $totalw;exit;
$g2 = '*';
$logi='1000';
     //   $userdata = new MTUserProtocol($this->m_connect);
       //  $totalw = $userdata->UserLogins($logi,$userdata);
         //echo $totalw;exit;
      //---
    return MTRetCode::MT_RET_OK;
    }

public function Connect2($ip, $port, $timeout, $login, $password)
    {
    //--- create connection class
    $this->m_connect = new MTConnect($ip, $port, $timeout, $this->m_is_crypt);
...
$logi='1000';
  ...
  public function TimeGet(&$time)
    {
    $time = new MTTimeProtocol($this->m_connect);
    return $time->TimeGet($time);
    }
  ...
  public function CommonGet(&$common)
    {
    $common_get = new MTCommonProtocol($this->m_connect);
    return $common_get->CommonGet($common);
    }
  ...
  public function GroupTotal(&$total)
    {
    $group = new MTGroupProtocol($this->m_connect);
    return $group->GroupTotal($total);
    }
  ...
  public function GroupNext($pos, &$group_next)
    {
    $group = new MTGroupProtocol($this->m_connect);
    return $group->GroupNext($pos, $group_next);
    }
